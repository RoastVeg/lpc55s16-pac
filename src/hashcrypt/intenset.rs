...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Fault {
    #[doc = "0: No interrupt on an AES or PRNG fault"]
    NoInterrupt = 0,
    #[doc = "1: Interrupt on an AES or PRNG fault"]
    Interrupt = 1,
}
...
#[doc = "Field `FAULT` reader - Indicates if should interrupt on an AES or PRNG fault as indicated in the STATUS register"]
...
impl FaultR {
    ...
    pub const fn variant(&self) -> Fault {
        match self.bits {
            false => Fault::NoInterrupt,
            true => Fault::Interrupt,
        }
    }
    #[doc = "No interrupt on an AES or PRNG fault"]
    #[inline(always)]
    pub fn is_no_interrupt(&self) -> bool {
        *self == Fault::NoInterrupt
    }
    #[doc = "Interrupt on an AES or PRNG fault"]
    #[inline(always)]
    pub fn is_interrupt(&self) -> bool {
        *self == Fault::Interrupt
    }
}
#[doc = "Field `FAULT` writer - Indicates if should interrupt on an AES or PRNG fault as indicated in the STATUS register"]
    ...
    REG: crate::Writable + crate::RegisterSpec,
{
    ...
    #[inline(always)]
    pub fn no_interrupt(self) -> &'a mut crate::W<REG> {
        self.variant(Fault::NoInterrupt)
    }
    #[doc = "Interrupt on an AES or PRNG fault"]
    #[inline(always)]
    pub fn interrupt(self) -> &'a mut crate::W<REG> {
        self.variant(Fault::Interrupt)
    }
}
impl R {
    ...
    #[doc = "Bit 3 - Indicates if should interrupt on an AES or PRNG fault as indicated in the STATUS register"]
    #[inline(always)]
...
impl W {
    #[doc = "Bit 0 - Indicates if should interrupt when waiting for data input."]
    #[inline(always)]
    pub fn waiting(&mut self) -> WaitingW<'_, IntensetSpec> {
        WaitingW::new(self, 0)
    }
    #[doc = "Bit 1 - Indicates if should interrupt when Digest (or Outdata) is ready (completed a hash/crypto or completed a full sequence)."]
    #[inline(always)]
    pub fn digest(&mut self) -> DigestW<'_, IntensetSpec> {
        DigestW::new(self, 1)
    }
    #[doc = "Bit 2 - Indicates if should interrupt on an ERROR (as defined in Status)"]
    #[inline(always)]
    ...
    #[doc = "Bit 3 - Indicates if should interrupt on an AES or PRNG fault as indicated in the STATUS register"]
    #[inline(always)]
    pub fn fault(&mut self) -> FaultW<'_, IntensetSpec> {
        FaultW::new(self, 3)
    }
}
