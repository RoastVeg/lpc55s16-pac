...
impl RegisterBlock {
    ...
    #[doc = "0x24..0x40 - Aliases to allow writing words in a burst."]
    #[inline(always)]
    ...
    #[doc = "0x40..0x60 - Result digest (when status says so): Is 1st 5 words if SHA1 used Is all 8 words if SHA2 used Is all 8 words if crypto or SHA512"]
    #[inline(always)]
    ...
    #[doc = "0x90..0xa0 - Allows Application to write a random mask for ICB use. Normally only a new one on each system reset (including power up)."]
    #[inline(always)]
    ...
    #[doc = "0xa0..0xc0 - The WO digest-reload registers may be written with a saved Hash digest, to allow continuation from where left off. These registers may only be written if the Reload field in CTRL is 1. If SHA1, only the 1st 5 are used."]
    #[inline(always)]
    ...
    #[doc = "0xd0 - PRNG random input value used as an entropy source"]
    #[inline(always)]
    pub const fn prng_seed(&self) -> &PrngSeed {
        &self.prng_seed
    }
    #[doc = "0xd8 - Provide random number."]
    #[inline(always)]
...
pub mod mask;
#[doc = "RELOAD (w) register accessor: The WO digest-reload registers may be written with a saved Hash digest, to allow continuation from where left off. These registers may only be written if the Reload field in CTRL is 1. If SHA1, only the 1st 5 are used.\n\nYou can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`reload::W`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).\n\nFor information about available fields see [`mod@reload`] module"]
#[doc(alias = "RELOAD")]
